(function () {
  ...
  async function init() {
    ...
    if (!state.storageAvailable) {
      ...
      savedCustomizations &&
      hasGuestCustomizations(savedCustomizations)
    ) {
      updateSaveStatus(
  ...
  function bindEvents() {
    const liveControls = [
      ...
      elements.intakeFilter,
      elements.portalFilter,
      elements.vpdFilter,
      elements.moiFilter,
      elements.restrictedFilter,
      elements.feeTypeFilter,
      elements.tuitionFeeMin,
      elements.tuitionFeeMax,
      elements.startDateFrom,
      elements.startDateTo,
      elements.appliedFilter
    ];
    ...
    elements.addProgramButton.addEventListener(
      "click",
      () => openProgramDialog()
    );
    ...
    elements.programTable.addEventListener(
      "click",
      handleTableClick
    );
    ...
    elements.programForm.addEventListener(
      "submit",
      saveProgramFromForm
    );
    ...
    elements.programDialog.addEventListener(
      "cancel",
      (event) => {
    ...
    elements.programTableWrap.addEventListener(
      "scroll",
  ...
  async function loadPublishedDataset() {
    try {
      ...
      throw new Error(
        "No tracker data could be loaded."
      );
    }
  }
    ...
    try {
      ...
      return {
        ...
        deletedIds: parsed.deletedIds
          ...
          .filter(Boolean),
        customPrograms:
          parsed.customPrograms.map(normalizeProgram),
      };
    } catch (error) {
      console.warn(
        "Saved guest data could not be read",
        error
      );
  ...
  function applyGuestCustomizations(
    publishedDataset,
    customizations
  ) {
    ...
    customizations.customPrograms.forEach(
      (program) => {
        if (
          !baseIds.has(program.id) &&
          !deletedIds.has(program.id)
        ) {
          programs.push(
            normalizeProgram(program)
  ...
  function hasGuestCustomizations(
    customizations
  ) {
    return Boolean(
      Object.keys(
  ...
  function clearGuestCustomizations() {
    try {
      ...
      console.warn(
        "Could not clear browser-saved guest data",
        error
      );
  ...
  function applyDataset(payload) {
    ...
    state.metadata = {
      title:
        normalized.title ||
        "Germany University Application Tracker (Summer)",
      currency:
        normalized.currency ||
        "EUR",
      schemaVersion:
  ...
  function normalizeDataset(payload) {
    ...
    if (
      !Array.isArray(sourcePrograms)
    ) {
      throw new Error(
        "JSON must contain a programs array."
      );
    }
  ...
  function createId(index = "") {
    ...
    return `program-${Date.now()}-${index}-${Math.random()
      .toString(16)
      .slice(2)}`;
  }
  ...
  function persistDataset() {
    const customizations =
      buildGuestCustomizations();

    try {
      if (
        hasGuestCustomizations(
          customizations
        )
      ) {
      ...
      console.warn(
        "Could not save guest data",
        error
      );

      if (
        !state.storageErrorShown
      ) {
        state.storageErrorShown =
          true;
  ...
  function getCriteria() {
    return {
      ...
      intake:
        elements.intakeFilter.value,

      portal:
        elements.portalFilter.value,

      vpd:
        elements.vpdFilter.value,

      moi:
        elements.moiFilter.value,

      restricted:
        elements.restrictedFilter.value,

      feeType:
        elements.feeTypeFilter.value,

      tuitionFeeMin:
        elements.tuitionFeeMin.value,

      tuitionFeeMax:
        elements.tuitionFeeMax.value,

      startFrom:
        elements.startDateFrom.value,

      startTo:
        elements.startDateTo.value,

      applied:
        elements.appliedFilter.value,
    };
  }
  ...
  function render() {
    ...
    const filtered =
      TrackerFilters.filterPrograms(
        state.programs,
        getCriteria()
      );

    const visiblePrograms =
      TrackerFilters.sortPrograms(
        filtered,
        state.sortField,
        state.sortDirection
      );
  ...
  function updateTableScrollHint() {
    const wrap =
      elements.programTableWrap;

    const hint =
      elements.tableScrollHint;

    const fade =
      elements.tableEdgeFade;
  ...
  function renderStats(
    ...
    elements.visibleCount.textContent =
      `${visibleLength.toLocaleString(
        "en-US"
      )} visible`;

    elements.freeTuitionCount.textContent =
      freeTuition.toLocaleString(
        "en-US"
      );

    elements.appliedCount.textContent =
      applied.toLocaleString(
        "en-US"
      );

    elements.appliedPercent.textContent =
      `${total
        ? Math.round(
          (applied / total) *
          100
    ...
    elements.vpdCount.textContent =
      vpd.toLocaleString(
        "en-US"
      );

    elements.resultSummary.textContent =
      `Showing ${visibleLength.toLocaleString(
        "en-US"
  ...
  function renderTable(programs) {
    ...
    elements.programTableBody.append(
      fragment
    );
  }
  ...
  function createProgramRow(
    ...
    checkbox.setAttribute(
      "aria-label",
      `Mark ${program.courseName} at ${program.universityName} as applied`
    );
  ...
  function openProgramDialog(
    ...
    if (program) {
      Object.entries(
        ...
        ([key, value]) => {
          const field =
            elements.programForm.elements.namedItem(
              key
            );
  ...
  function saveProgramFromForm(event) {
    ...
    if (
      !elements.programForm.reportValidity()
    ) {
      return;
    }
  ...
  function deleteProgram(program) {
    if (
      !confirm(
        `Delete ${program.courseName} at ${program.universityName}?`
      )
    ) {
      return;
    }
  ...
  function toggleFilters() {
    ...
    elements.filterToggle.textContent =
      willHide
        ? "Show Filters"
        : "Hide Filters";
  }
  ...
  async function resetDataset() {
    ...
    if (storageCleared) {
      ...
      showToast(
        "Your browser-saved guest data was deleted."
      );
    } else {
      updateSaveStatus(
        "Reset for this page",
  ...
  async function exportFilteredPdf() {
    ...
    if (
      ...
      showToast(
        "There are no visible programs to export.",
        true
      );
    ...
    try {
      ...
      showToast(
        `PDF downloaded with ${visiblePrograms.length} visible program${visiblePrograms.length === 1
          ? ""
          : "s"
        }.`
      );
  ...
  function buildProgramsPdf(
    ...
    for (
      ...
      stream += pdfText(
        34,
        40,
        `${programs.length} currently visible program${programs.length === 1
          ? ""
          : "s"
      ...
      stream += pdfText(
        MARGIN,
        PAGE_HEIGHT - 12,
        "Maintained and Developed by Boishik Barua Tinu",
        7.3,
        "F1",
        [98, 125, 152],
        PAGE_HEIGHT
  ...
  function getPdfFilterSummary() {
    ...
    if (
      criteria.tuitionFeeMin
    ) {
      parts.push(
        `Minimum tuition fee = EUR ${criteria.tuitionFeeMin}`
      );
    }

    if (
      criteria.tuitionFeeMax
    ) {
      parts.push(
        `Maximum tuition fee = EUR ${criteria.tuitionFeeMax}`
      );
    }
    ...
    return parts.length
      ? parts.join(" | ")
      : "None - all programs are included";
  }
    ...
    pageStreams.forEach(
      (
        ...
        objects[
          ...
          `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pdfNumber(pageWidth)} ${pdfNumber(pageHeight)}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents ${contentObjectId} 0 R >>`;
